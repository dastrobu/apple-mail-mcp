...
// Simulate Cmd+A keystroke
static void SimulateCmdA() {
    CGEventSourceRef source = CGEventSourceCreate(kCGEventSourceStateHIDSystemState);
    if (!source) return;

    // key code 0 is 'a' (ANSI A)
    CGKeyCode aKey = 0;

    // Create KeyDown event
    CGEventRef keyDown = CGEventCreateKeyboardEvent(source, aKey, true);
    if (keyDown) {
        CGEventSetFlags(keyDown, kCGEventFlagMaskCommand);
        CGEventPost(kCGHIDEventTap, keyDown);
        CFRelease(keyDown);
    ...
    // Create KeyUp event
    CGEventRef keyUp = CGEventCreateKeyboardEvent(source, aKey, false);
    if (keyUp) {
        CGEventSetFlags(keyUp, kCGEventFlagMaskCommand);
        CGEventPost(kCGHIDEventTap, keyUp);
        CFRelease(keyUp);
...
// IsTrusted checks if the current process has Accessibility permissions
func IsTrusted() bool {
	return bool(C.IsProcessTrusted())
}
...
2. If it's already enabled, remove the application using the - symbol
3. On your next attempt to use the tool, you will get prompted again for the current app version.
4. IMPORTANT: After granting permission, you MUST restart the service for it to take effect.
...
// SimulateSelectAll simulates Cmd+A keystroke using CoreGraphics
func SimulateSelectAll() {
	// Cmd+A is handled by SimulateCmdV logic but with 'a' (0) instead of 'v' (9)
	// We should reuse logic or implement it here.
	// Since SimulateCmdV is hardcoded to 'v', let's implement SimulateCmdA in C or add a generic SimulateCmdKey.
	// For simplicity, I'll add SimulateCmdA to CGO block.
	C.SimulateCmdA()
}
